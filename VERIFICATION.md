...
**Runs today:** `make check` exits 0 with **6 real assertions passing and 28 loud `PENDING` skips**
(46 deselected — 43 browser checks and 3 GE checks, which belong to the layers below).
...
Composes four layers, in the order that fails cheapest first:
...
Idempotent, installs nothing, safe on every session start, and **no skip knobs** — a boot script
whose last line says `ready.` has to mean it. Three things it does that a friendlier script would
not:
...
There is no toolchain preflight loop: `make check` reports a missing `ruff`/`mypy`/`pytest` two
seconds later by itself, and a hand-listed loop is one more thing to keep in sync. `init.sh` does
`cd` to its own directory, because every script here reads `./.env` relative to CWD.
...
The gate governs `app/` and `tests/`. It **excludes `learning-tests/` and `seed/`**.
...
**Dependency model:** per-script `uv run --with …`, no lockfile, dependencies declared in each
script's `RUN` docstring block. `pyproject.toml` exists only to configure the gate and declares
no dependencies and no build backend. *Ceiling:* adding a resolver and a lockfile before there
is an app to lock would be scaffolding for later. The trigger to formalise is the first time two
modules need the same non-stdlib import at runtime.
...
axe-core injected into the page and run in-page; fail on serious/critical, log moderate.
**Needs installing:** the axe-core bundle, vendored as one file (no npm dependency on the Python
side — the entire Node/Next stack is currently uninstalled). Until it is vendored this check is
`PENDING`, not silently green — and it deliberately does **not** request the `driver` fixture,
because that fixture pends first and would mask this reason with *"APP_URL is unset"*. The
blocker here needs a human to act, so it has to be the reason that reaches the `-ra` output.
...
| file size | stdlib in `tests/test_code_quality_thresholds.py`, threshold 400 lines | **on** |
...
| duplication | `jscpd` (Node) — **NOT INSTALLED**, and Node has no `package.json` here | **off** |
| unused exports | `knip` / `ts-prune` — **NOT INSTALLED**, needs the Next.js side to exist | **off** |
...
behaviour and rots when the default moves. Duplication and unused-export detection are deliberately off: with zero application
files and no `package.json`, both would report on an empty set. **Trigger to add them:** the
first `npm install` — `knip` and `jscpd` go in with the frontend, not before.
...
|---|---|
| **Duplication and unused-export detection** | Zero app files and no `package.json`; both would scan an empty set. Trigger: the first `npm install`. |
...
| **Security scanning (SAST, dependency audit)** | No dependency manifest exists. Trigger: the first lockfile. |
...
| **Cumulative layout shift** | Listed in the hygiene file as unclaimed so it stays a decision. Add if a CLS problem actually appears. |
